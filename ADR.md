...
- **The polling endpoint reads a third-party model.** The dashboard needs the last N results across every queue, and the Tasks Framework only exposes `TaskResult` per task id - no equivalent for that listing. So it reads `django_tasks_db.models.DBTaskResult`, the package's storage model, not a public API. That bet is contained in `desk/task_feed.py`: the import, the queries and the column names (`task_path`, `exception_class_path`, `enqueued_at`) all live behind that boundary, so a schema change upstream breaks one module rather than the view, and the dashboard's JSON contract doesn't inherit the package's column names. It remains a coupling worth watching - if `django-tasks-db` ever grows a public query API, `task_feed` is the single place to switch over.
